...
// Photo (or slideshow) with a dark-green caption band; on hover the band rises to reveal a short description.
export default function CaptionCard({ image, label, title, text, to = '/contact', aspect = 'aspect-[4/3]', className = '' }) {
  return (
    <Link to={to} className={`group relative block overflow-hidden bg-forest ${className}`}>
      <Slides images={image} className={`${aspect} w-full`} zoom="transition-transform duration-[1600ms] ease-out group-hover:scale-110" />
